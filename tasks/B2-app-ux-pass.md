...
- **People are grammatical, not illustrated guesses:** TIPNR v4 classifies 3,132 people and resolves all 9,461 source relationship edges as parent, sibling, partner, or offspring while retaining 251 uncertain identifications. Role, era, affiliation, compact family rows, dense-group disclosure, related-person activation, and session back/forward history replace speculative portrait space.
- **Media claims are explicit:** all 604 packaged images are conservatively labelled proposed site, geographic context, associated artifact, or later reception. Each disclosure names what is depicted, creator, Commons/source, and exact license; the renderer still permits no remote image loads.
- **Pleiades is pinned and separate:** release 4.1 (2025-05-28, fixed commit and DOI, CC BY 3.0) contributes 96 linked ancient records, 401 historical names, 124 geometries, 118 place connections, and 886 bibliography rows. OpenBible modern geography and Pleiades ancient assertions remain separate, with coordinate comparison visible instead of silently choosing a winner.
- **Maps explain themselves:** the minimap labels OpenBible, visible Pleiades points, and alternative proposals; broad places get regional framing and representative-point language; the 96 TIPNR places without an OpenBible join receive a deliberate unmapped state.
- **Scripture remains usable:** distribution and book-grouped reference rows make small and 900+ occurrence entities legible. Every reference remains clickable, scrolls its selected verse to the reading eye-line, and keeps the entity open.
- **Refusing import and repeatable proof:** the Pleiades fetch pins exactly the 96 linked raw records outside Git, then the importer verifies release manifest, hashes, identities, coordinates, geometry, rights, richness, comparison coverage, and deterministic normalized output. `verify:data` rechecks both place Doctors and artifact checksums. The self-driving tour covers Corinth, Capernaum, broad-place coordinate comparison, an unmapped place, all four media kinds, sparse/dense people, family history, book grouping, reference navigation, provenance, overflow, and Paper/Ink/Glass/Candlelight captures.

**Verification:** focused TIPNR/place tests, `npm run verify:data`, `npm run lint`, Electron and renderer builds, `npm run qa:entities`, visual inspection across all four desktop atmospheres, and `git diff --check`. Mobile remains outside this worktree.
...
- **Complete structured-row parsing:** TIPNR v4 unions every one of the **5,810** reference-bearing name-form rows instead of choosing the longest row. It recognizes all shipped significance types, carries abbreviated book/chapter/comma context, preserves LXX identity, normalizes verse ranges, and records the two pinned source-coordinate corrections (`Psa.68.36 → Psa.68.35`, `Mat.15.42 → Mat.12.12`) rather than accepting invalid coordinates.
...
- **Dependent data stays aligned:** all **917** OpenBible place records are resynchronized byte-for-byte to the canonical TIPNR reference lists. The place Doctor now refuses TIPNR-reference drift.
...
- **Deterministic biblical geography:** a pinned OpenBible Bible Geocoding snapshot is joined to the shipped TIPNR identities. The normalized artifact maps **917 of 1,013** TIPNR places, retaining exact OpenBible identity, coordinates, confidence score, precision, alternative proposals, Scripture references, and available Pleiades and Wikidata identifiers. Unmapped places remain honest TIPNR research cards with no invented coordinate.
- **Licensed visual context:** **604 place records** have a local selected image from the packaged source set. Each record keeps semantic kind, depicted subject, creator, source, license, license URL, alt text, dimensions, and SHA-256. Unapproved image licenses are not shipped and fall back to the map treatment; people use a quiet typographic identity treatment rather than a speculative portrait.
...
- **Translation-free selection persists:** whole-verse selection and its stable range anchor survive BSB/WEB changes because their coordinates are canonical. The version trigger and its portaled menu are selection-preserving controls rather than click-away targets; only the transient toolbar, translation-specific phrase offsets, and highlight animations are cleared.
