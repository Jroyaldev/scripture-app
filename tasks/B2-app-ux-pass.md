...
- **Scope and lens are orthogonal:** the persistent `Chapter | In view | Selected` state still follows the reading context, while `Cross refs | Passage | Notes` remains the reader's stable view choice across verse and scope changes. Cross refs is leftmost and opens by default; a deliberate reader choice still persists afterward.
- **Three purpose-built views:** Cross refs owns OpenBible relationships and visibly separate note-derived links. Passage owns overview, original-language study, word detail, and highlight tools. Notes owns passage capture, anchored notes, insight, themes, claims, and related-note evidence.
...
- **Purposeful inner hierarchy:** Cross refs begins directly with the attributed OpenBible source instead of repeating a generic Connections heading. Passage names its highlight action and current state. Notes uses `In this chapter`, `At this verse`, or `For this passage` rather than echoing the tab label.
