...
| `spoken-gospel-emblem-gold.svg` | Their **cover art**'s bottom-left corner | The emblem in the ink they actually use. What shipped was `spoken-gospel-emblem-white.png`, cut from their one-ink lockup and reversed to white; their cover sets both wordmark and emblem in **gold leaf on navy**. Lifted from the corner of the cover and traced. `#EAD36E` is the *mean of the leaf* — gold leaf is a texture, not a flat colour — rather than a gold chosen by us. 9.35:1 on their navy. Nothing redrawn. 0.888. |
| `five-minutes-numeral-white.svg` | Their **cover art**'s display numeral | **Replaces `ligonier-tree-white.svg` on this source.** The inventory already stated the problem: a device identifies the PUBLISHER, not the show, and Ligonier's tree was accepted only because it sat *beside* the title and left it standing. On a square shelf the mark is alone in its cell with no title beside it, so the tree says Ligonier and stops. The show's own numeral takes its place — traced from their cover, reversed to one ink, 6.35:1 on their ground — and it draws a family that was always implied and never rendered: 5, 30 and 40 Minutes each wearing their own numeral. 0.548. |
| `ask-nt-wright-bubble.svg` | Their **cover art**, not their lockup | The bubble lifted out of the show's own 3000px cover by flooding the field in from the border — a colour key would have punched holes in the letters, which sit close in value to the field behind them — then traced to two paths, the silhouette and the type, so the letters stay crisp rather than becoming the seam between two traced regions. Both inks measured off the artwork: `#FFFFFF` and `#2080A0`. Nothing redrawn. 1.404. **Replaces `ask-nt-wright-bubble-white.png`**, which was cut from their horizontal lockup and is their OTHER treatment of the same mark: teal bubble, white type, tail pointing DOWN. That crop was never corrupted — a fresh cut from the lockup is pixel-identical to it — it was the wrong one of their two, because the cover is the artwork a listener knows the show by, and on the cover the bubble is white with teal type and the tail points UP. |
