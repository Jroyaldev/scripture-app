...
- Scope and lens are independent. `Chapter | In view | Selected` follows the reading context automatically; `Overview | Refs | Passage | Notes` is the reader's stable choice and does not reset merely because the reference changes. Overview is leftmost and opens by default.
- Overview is a selective intent surface: at most two ranked OpenBible references, the strongest grounded local-library evidence, and up to four exact TIPNR people/place entries. It may show less or nothing; it must not manufacture relationship types, importance, or biography.
- Refs owns the complete returned OpenBible relationships and a separately named local note-derived section. Passage owns chapter context, original-language study, word detail, and highlight tools. Notes owns capture, anchored notes, passage insight, related notes, themes, grounded claims, and complete expandable note bodies.
- Reference and bounded quotation stay above the tabs so all four lenses retain context. Each lens restores its own scroll position; changing scope clears stale scroll positions while preserving the chosen lens.
...
- Lens content does not repeat the selected tab as a generic heading. Overview uses only the source-shaped groups `Scripture`, `Your library`, and `People & places`; Refs begins with source and scope, Passage names the highlight action and state, and Notes names the current chapter/verse/passage scope.
