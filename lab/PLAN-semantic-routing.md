...
### 13. Middle shaft + approach-side grammar — *adapt* (the one constraint amendment; last on purpose) — ✅ LANDED
...
### C0.3. Bidirectional whole-margin routing — *adapt* — ✅ LANDED
- **Build:** derive stable `leftLoomInner`/`rightLoomInner` from the complete obstacle datum (`loomX` remains the left alias); reflect the full margin grammar with directional pins, max-right level-member selection, comb ordering, turns, shoulders, swoops, ports, spines, and C0.2 claims. Search the first legal strand independently per side. Compare whole candidates by actual ink + semantic-side cost + 12-unit previous-side hysteresis; tenth-pixel quantization defines a calm tie, then requested-side order and strand decide. Unsupported sides decline. Only painted real margin plans commit claims or update side memory.
- **Host:** route-lab and Shapes request both margins and expose side/strand metadata. Held ticks project from valid shadow plans, use deterministic prior-side/left fallback otherwise, remain bounded, and retain dimension-matched phantom positions plus exact annotation/line roving focus. Pointer activation completes on `pointerup` with a movement threshold; keyboard activation transfers focus to the selected fixture/score row. The right sheet edge remains inside the existing 24px score gap; no second margin or score redesign.
...
- **Manual Intelligent/left/right mode select** — route choice is now automatic geometry; a manual rail selector would expose implementation state without improving the reading act.
